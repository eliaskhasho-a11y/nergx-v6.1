...
function toast(msg,type=''){ const wrap = document.getElementById('toasts'); const el = document.createElement('div'); el.className = 'toast '+type; el.textContent = msg; wrap.appendChild(el); setTimeout(()=>el.remove(), 2800); }
...
function makeSortable(table, data, columns){
  // columns = [{key:'name',selector:'th:nth-child(2)'}...]
  ...
  columns.forEach((c, idx)=>{
    ...
    th.addEventListener('click', ()=>{
      ...
      // trigger re-render via provided callback (table lives inside current view → call the view again)
      const route = resolveRoute(); routes[route](document.getElementById('app'));
    });
  });
}
...
function dashboardView(el){
  el.innerHTML = `
    <h1>Översikt <span class="badge small">v8.8 Online</span></h1>
    <section class="kpis">
      <div class="kpi"><div>Omsättning idag</div><div class="v mono" id="kpiRevenue">324 500 kr</div><div class="muted">+12 % mot igår</div></div>
...
function crmView(el){
  ...
  function filteredData(filter=''){
    ...
    return rows.filter(r =>
      [r.code,r.name,r.contact,r.email,r.phone,r.city].join(' ').toLowerCase().includes(f)
    );
  }
  function renderRows(){
    const data = filteredData(search.value);
  ...
  // Sortering
  makeSortable(document.getElementById('custTable'), rows, [
    {key:'name', selector:'th:nth-child(2)'},
    {key:'city', selector:'th:nth-child(6)'},
  ]);
  ...
  tbody.addEventListener('click', e=>{
    ...
    if(e.target.hasAttribute('data-del')){
      if(confirm('Ta bort kund?')){
        ...
        rows.splice(i,1); toast('Kund borttagen','bad'); renderRows();
      }
    }
  });
  ...
  document.getElementById('cancelCust').onclick = ()=> document.getElementById('custFormCard').style.display='none';
  document.getElementById('saveCust').onclick = ()=>{
    const name = document.getElementById('c_name').value.trim();
    const email = document.getElementById('c_email').value.trim();
    if(!name){ toast('Företagsnamn krävs','bad'); return; }
    if(email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){ toast('Ogiltig e‑post','warn'); return; }
    const payload = {
      name,
      orgnr:document.getElementById('c_org').value.trim(),
      contact:document.getElementById('c_contact').value.trim(),
      email, phone:document.getElementById('c_phone').value.trim(), city:document.getElementById('c_city').value.trim()
    };
    if(!editId){
      DB.customers.push({ id:uuidv4(), code:'C'+(1000+DB.customers.length+1), ...payload });
      toast('Kund skapad','ok');
    } else {
      const i = DB.customers.findIndex(x=>x.id===editId);
      DB.customers[i] = { ...DB.customers[i], ...payload };
      toast('Kund uppdaterad','ok');
    }
    document.getElementById('custFormCard').style.display='none';
    renderRows();
  };
}
...
function inventoryView(el){
  ...
  function filteredData(filter=''){
    const f = filter.toLowerCase();
    return rows.filter(r =>
      [r.sku,r.name,String(r.price),String(r.stock)].join(' ').toLowerCase().includes(f)
    );
  }
  function renderRows(){
    const data = filteredData(search.value);
  ...
  makeSortable(document.getElementById('prodTable'), rows, [
    {key:'sku', selector:'th:nth-child(1)'},
    {key:'name', selector:'th:nth-child(2)'},
    {key:'price', selector:'th:nth-child(3)'},
    {key:'stock', selector:'th:nth-child(4)'},
  ]);

  // Inline-edit with type casting
  tbody.addEventListener('dblclick', e=>{
    const td = e.target.closest('.cell-edit'); if(!td) return;
    const tr = td.closest('tr'); const id = tr.getAttribute('data-id');
    const key = td.getAttribute('data-key'); const obj = rows.find(x=>x.id===id);
    inlineEdit(td, obj[key]??'', (v)=>{
      let val = v;
  ...
  tbody.addEventListener('click', e=>{
    ...
    if(e.target.hasAttribute('data-del')){
      if(confirm('Ta bort produkt?')){
        const i = rows.findIndex(x=>x.id===id);
        rows.splice(i,1); toast('Produkt borttagen','bad'); renderRows();
      }
    }
  });
  ...
  document.getElementById('addProd').onclick = ()=>{
    editId=null;
    document.getElementById('prodFormTitle').textContent='Ny produkt';
    ['p_sku','p_name','p_price','p_stock'].forEach(idf=>document.getElementById(idf).value='');
    document.getElementById('prodFormCard').style.display='block';
  };
  document.getElementById('cancelProd').onclick = ()=> document.getElementById('prodFormCard').style.display='none';
  document.getElementById('saveProd').onclick = ()=>{
    ...
    const payload = { sku,name,price,stock };
    if(!editId){ rows.push({ id:uuidv4(), ...payload }); toast('Produkt skapad','ok'); }
    else { const i = rows.findIndex(x=>x.id===editId); rows[i] = { ...rows[i], ...payload }; toast('Produkt uppdaterad','ok'); }
    document.getElementById('prodFormCard').style.display='none'; renderRows();
  };
}
  ...
  el.innerHTML = `
    ...
    <div class="card">
      ...
      <table class="table" style="margin-top:10px">
        <thead><tr><th>Ordernr</th><th>Kund</th><th>Status</th><th>Total</th><th>Betalvillkor</th></tr></thead>
...
function employeesView(el){
  ...
  el.innerHTML = `
    ...
    <div class="card">
      ...
      <table class="table" style="margin-top:10px">
        <thead><tr><th>ID</th><th>Namn</th><th>Roll</th><th>Email</th><th></th></tr></thead>
  ...
  tbody.addEventListener('click', e=>{
    ...
    if(e.target.hasAttribute('data-del')){
      if(confirm('Ta bort anställd?')){
        const i = rows.findIndex(x=>x.id===id); rows.splice(i,1); renderRows(); toast('Anställd borttagen','bad');
      }
    }
  });
...
async function aiMapView(el){
  el.innerHTML = `<h1>AI‑Karta</h1><div id="map"></div>`;
  try{
    if(!('L' in window)){
      await Promise.all([
...
function automationView(el){
  el.innerHTML = `
    <h1>Automation</h1>
    <div class="card">
      <p class="muted">Trigger → Villkor → Åtgärd (mock). Ex: Ny order ⇒ skicka tracking via WhatsApp/SMS (UI‑stub).</p>
      <div class="actions"><button class="btn">Lägg till flow</button><button class="btn ghost">Importera</button></div>
    </div>
  `;
}
...
// ====== Dock (AI-coach med kontext) ======
function initDock(){
  const n = document.getElementById('noticeList');
  ['v8.8 laddad','AI‑coach aktiv','CRUD, sök, sortering, inline‑edit'].forEach(x=>{
  ...
  function aiReply(){
    const low = DB.products.filter(p=>p.stock<10).map(p=>p.sku).join(', ') || 'inga artiklar';
    const c = DB.customers.length, o = DB.orders.length;
    return `AI: Du har ${c} kunder, ${o} order och ${low==='inga artiklar'?'inga låglagerartiklar':('lågt lager på: '+low)}. Rek: skapa inköpsorder och följ upp B2B‑kunder i Stockholm.`;
  }
  function send(){
    ...
    input.value='';
    setTimeout(()=>{
      const ai = document.createElement('div'); ai.textContent = aiReply(); msgs.appendChild(ai);
