...
const Notis = {
  init(){
    ...
    this.push('MergX v8.0.1 patch laddad.');
    setInterval(()=>{
      if(Math.random()<0.22){
...
const Coach = {
  init(){
    ...
    if(!this.wrap) return;
    this.say('Hej! Jag är din AI-coach. Jag reagerar på KPI, lager, deadlines och schema.', true);
    this.form.addEventListener('submit', (e)=>{
      ...
      setTimeout(()=>{
        ...
        if(/lager|stock/i.test(q)) a = `Brist på ${State.stockAlerts.map(s=>s.product).join(', ')}. Förslag: inköp idag + påminnelse.`;
        ...
        if(/karta|map/i.test(q)) a = `Hög potential i Stockholm & Tammerfors. Rutt finns i AI-Karta.`;
        this.say(a, true);
      }, 500);
    });
